...
export default function (value, length = 0, char = ' ') {
    ...
    const leftLength = Math.ceil((length - value.length) / 2);
    const rightLength = length - value.length - leftLength;

    return repeat(char, leftLength) + value + repeat(char, rightLength);
}
